...
void mainImage(thread float4& fragColor, float2 fragCoord, float iTime, float3 iResolution)
{
    ...
    float3 col = float3( 0.0, 0.3, 0.4 );

    float f = fbm( 5.0*p );
    col = mix( col, float3(0.2,0.5,0.4), f );

    col = mix( col, float3(0.9,0.6,0.2), 1.0-smoothstep(0.2,0.6,r) );

    a += 0.05*fbm( 20.0*p );

    f = smoothstep( 0.3, 1.0, fbm( float2(20.0*a,6.0*r) ) );
    col = mix( col, float3(1.0,1.0,1.0), f );
    ...
    f = 1.0-smoothstep( 0.0, 0.6, length2( float2x2(0.6,0.8,-0.8,0.6)*(p-float2(0.3,0.5) )*float2(1.0,2.0)) );

    col += float3(1.0,0.9,0.9)*f*0.985;

    col *= float3(0.8+0.2*cos(r*a));

    f = 1.0-smoothstep( 0.2, 0.25, r );
    col = mix( col, float3(0.0), f );

    f = smoothstep( 0.79, 0.82, r );
    col = mix( col, float3(1.0), f );
