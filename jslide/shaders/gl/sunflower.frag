...
, vec3 mountainColor){
    ...
    mountainColor +=(1.-randTwoD((uv + vec2(movement, 0.))*1.5)).x*0.1;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    DrawMountain(totalMovementSpeed * 1., uv, col, 0.5, 0.08, -0.135, 0.4, 0., 2.723, MountainLayerFourCol);
     ...
     fieldBaseColor = mix(FieldMid + pow((randTwoD((uv + vec2(movement * 4.7,0.))*1.2)).x ,   2.)*0.05   ,
              SunflowerLeavesOne + pow((randTwoD((uv + vec2(movement*1.,0.))*1.2)).x ,   2.)*0.05
              , smoothstep(-0.2, 0.0, uv.y));
     fieldBaseColor = mix(fieldBaseColor,
     FieldDark + pow((randTwoD((uv + vec2(movement*6.2,0.))*1.2)).x ,   2.)*0.05
