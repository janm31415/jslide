...
C_Material GetObjectMaterial( const in vec3 vObjId, const in vec3 vPos )
{
	...
	{
		// floor
		vec4 cTextureSample = vec4(0.8, 0.4, 0.8, 1.0);                    
		mat.fR0 = 0.02;
	...
	if(vObjId.x < 2.5)
	{
		...
		vec4 cTextureSample = vec4(0.2, 0.8, 0.0, 1.0);
...
vec3 GetSkyGradient( const in vec3 vDir )
{
	...
	return mix(vec3(0.0, 0.0, 0.0), vec3(1, 1, 1), fBlend);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec2 mouse = vec2(0.5+0.01*sin(iTime*0.5), 0.5+0.01*cos(iTime*0.5));
	
		
	vec3 vCameraPos = OrbitPoint(-mouse.x * 14.0 + PI, (mouse.y) * PI * 0.2 + PI * 0.025) * 7.0 - vec3(0.0, 0.9+cos(iTime)*0.2, 0.0);
	#ifdef ENABLE_MONTE_CARLO              
