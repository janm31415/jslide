...
float dist(float3 p, float3 bpos[object_count])//distance function
{
	float d=1024.0;
	float nd;
...
float3 normal(float3 p,float e,float3 bpos[object_count]) //returns the normal, uses the distance function
{
	float d=dist(p,bpos);
...
float3 background(float3 d, float iTime)//render background
{
	...
	float qq = dot(d,light_direction)*.5+.5;
	float bgl = qq;
...
float occlusion(float3 p, float3 d, float3 bpos[object_count])//returns how much a point is visible from a given direction
{
	float occ = 1.0;
	p=p+d;
...
float3 object_material(float3 p, float3 d,float iTime, float3 bpos[object_count])
{
	...
	float diffuse = dot(light_direction,n)*.5+.5; diffuse = max(.0,diffuse);
	
	#ifdef occlusion_enabled
		float oa = occlusion(p,n,bpos)*.4+.6;
		float od = occlusion(p,light_direction,bpos)*.95+.05;
