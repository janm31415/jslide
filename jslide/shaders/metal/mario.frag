...
constant float MARIO_SPEED = 89.0;
constant float GOOMBA_SPEED = 32.0;
constant float INTRO_LENGTH = 2.0;

void SpriteBlock( thread float3& color, float x, float y )
{
    // black
    float idx = 1.0;
...
void SpriteHill( thread float3& color, float x, float y )
{
    float idx = 0.0;
...
void SpritePipe( thread float3& color, float x, float y, float h )
{
    float offset = h * 16.0;
...
void SpriteCloud( thread float3& color, float x, float y, float isBush )
{
float idx = 0.0;
...
void SpriteFlag( thread float3& color, float x, float y )
{
float idx = 0.0;
...
void SpriteCastleFlag( thread float3& color, float x, float y )
{
float idx = 0.0;
...
void SpriteGoomba( thread float3& color, float x, float y, float frame )
{
float idx = 0.0;
...
void SpriteKoopa( thread float3& color, float x, float y, float frame )
{    
float idx = 0.0;
...
void SpriteQuestion( thread float3& color, float x, float y, float t )
{
float idx = 0.0;
...
void SpriteMushroom( thread float3& color, float x, float y )
{
float idx = 0.0;
idx = y == 15.0 ? ( x <= 7.0 ? 40960.0 : 10.0 ) : idx;
...
void SpriteGround( thread float3& color, float x, float y )
{   
float idx = 0.0;
...
void SpriteFlagpoleEnd( thread float3& color, float x, float y )
{   
float idx = 0.0;
...
void SpriteMario( thread float3& color, float x, float y, float frame )
{    
    float idx = 0.0;
...
void SpriteCoin( thread float3& color, float x, float y, float frame )
{    
    float idx = 0.0;
if ( frame == 0.0 )
    {
...
void SpriteBrick( thread float3& color, float x, float y )
{    
float ymod4 = floor( fmod( y, 4.0 ) );    
    float xmod8 = floor( fmod( x, 8.0 ) );
    float ymod8 = floor( fmod( y, 8.0 ) );
...
void DrawCastle( thread float3& color, float x, float y )
{
if ( x >= 0.0 && x < 80.0 && y >= 0.0 && y < 80.0 )
{
float ymod4    = fmod( y, 4.0 );
float xmod8    = fmod( x, 8.0 );
float xmod16_4 = fmod( x + 4.0, 16.0 );
float xmod16_3 = fmod( x + 5.0, 16.0 );
float ymod8    = fmod( y, 8.0 );
...
void DrawKoopa( thread float3& color, float x, float y, float frame )
{
    if ( x >= 0.0 && x <= 15.0 )
    {
...
void KoopaWalk( thread float3& color, float worldX, float worldY, float time, float frame, float startX )
{
    float x = worldX - startX + floor( time * GOOMBA_SPEED );
    DrawKoopa( color, x, worldY - 16.0, frame );    
}

void DrawHitQuestion( thread float3& color, float questionX, float questionY, float time, float questionT, float questionHitTime )
{
...
void DrawW( thread float3& color, float x, float y )
{
...
void DrawO( thread float3& color, float x, float y )
{
    if ( x >= 0.0 && x < 14.0 && y >= 0.0 && y < 14.0 )
    {
...
void DrawR( thread float3& color, float x, float y )
{
    if ( x >= 0.0 && x < 14.0 && y >= 0.0 && y < 14.0 )
    {
        if (    ( x <= 3.0 )
...
void DrawL( thread float3& color, float x, float y )
{
    if ( x >= 0.0 && x < 14.0 && y >= 0.0 && y < 14.0 )
    {
        if ( x <= 3.0 || y <= 1.0 )
...
void DrawD( thread float3& color, float x, float y )
{    
    if ( x >= 0.0 && x < 14.0 && y >= 0.0 && y < 14.0 )
    {
    color = RGB( 255, 255, 255 );        
...
void Draw1( thread float3& color, float x, float y )
{    
    if ( x >= 0.0 && x < 14.0 && y >= 0.0 && y < 14.0 )
    {
        if (    ( y <= 1.0 )
...
void DrawM( thread float3& color, float x, float y )
{    
    if ( x >= 0.0 && x < 14.0 && y >= 0.0 && y < 14.0 )
    {
        if ( y >= 4.0 && y <= 7.0 )
...
void DrawIntro( thread float3& color, float x, float y, float screenWidth, float screenHeight )
{
    color = RGB( 0, 0, 0 );
...
void DrawGame( thread float3& color, float time, float pixelX, float pixelY, float screenWidth, float screenHeight )
{
    ...
    float worldXMod16= fmod( worldX, 16.0 );
    float worldYMod16 = fmod( worldY, 16.0 );
    ...
    // draw hills
    float bigHillX  = fmod( worldX, 768.0 );
    float smallHillX = fmod( worldX - 240.0, 768.0 );
...
float sc1CloudX = fmod( worldX - 296.0, 768.0 );
    float sc2CloudX = fmod( worldX - 904.0, 768.0 );
    float mcCloudX  = fmod( worldX - 584.0, 768.0 );
    float lcCloudX  = fmod( worldX - 440.0, 768.0 );    
    float scCloudX  = min( sc1CloudX, sc2CloudX );
    float sbCloudX = fmod( worldX - 376.0, 768.0 );
    float mbCloudX  = fmod( worldX - 664.0, 768.0 );  
float lbCloudX  = fmod( worldX - 184.0, 768.0 );
...
SpritePipe( color, pipeX, pipeY, pipeH );
    ...
    // draw coins
    float coinFrame = floor( fmod( time * 12.0, 4.0 ) );
...
float questionT = clamp( sin( time * 6.0 ), 0.0, 1.0 );    
   ...
   if (    ( tileY == 4.0 && ( tileX == 19.0 || tileX == 21.0 || tileX == 23.0 || tileX == 77.0 || tileX == 79.0 || tileX == 94.0 || tileX == 118.0 || tileX == 168.0 || tileX == 169.0 || tileX == 171.0 ) )
    ...
    // draw Koopa
    float goombaFrame = floor( fmod( time * 5.0, 2.0 ) );
...
goombaLifeTime = 23.5;
    ...
    {
        goombaLifeTime = 10.3;
goombaX = worldX - 790.0 - floor( abs( fmod( ( min( time, goombaLifeTime ) + 6.3 ) * GOOMBA_SPEED, 2.0 * 108.0 ) - 108.0 ) );
    }
goombaFrame = time > goombaLifeTime ? 2.0 : goombaFrame;
    ...
    goombaFrame = floor( fmod( time * 5.0, 2.0 ) );
    ...
    if ( goombaX < 0.0 ) goombaX = worldX - 766.0 - floor( abs( fmod( ( time + 6.3 ) * GOOMBA_SPEED, 2.0 * 108.0 ) - 108.0 ) );
    if ( goombaX < 0.0 ) goombaX = worldX - 638.0 - floor( abs( fmod( ( time + 6.6 ) * GOOMBA_SPEED, 2.0 * 84.0 ) - 84.0 ) );
...
float t = ( time2 - marioJumpTime ) / marioJumpLength;
    ...
    float marioFrame = marioJumpOffset == 0.0 ? floor( fmod( time * 10.0, 3.0 ) ) : 3.0;
    ...
    if ( fmod( t, 0.2 ) <= mix( 0.0, 0.2, clamp( t / 1.3, 0.0, 1.0 ) ) )
        {
            // super mario offset
            marioFrame += 4.0;
        }
